...
| EVM oracle pusher wallet unfunded | ⚠️ Dead since 2026-07-08 (out of testnet gas). Contract itself confirmed live on-chain. Pusher should be revived since it feeds the shared oracle. |
...
| RWA framing / `get_rwa_settlement_signal` | ✅ ADDED (2026-07-21) — responding to Casper's July 15, 2026 announcement about preparing for Kraken availability (part of its stated RWA/institutional push) and the buildathon's own stated focus on "DeFi and/or real-world assets." Added a third MCP tool combining the existing, already-verified network-safety signal with a factual description of Silicon DNA's identity-screening gate on the paid endpoint — no new unverified claims, just an honest recombination of two things already built and tested this session. Verified live: caught a real high-revert edge case (`arb_revert_pct: 15.73%`) correctly returning `network_safe_to_settle: false`. Casper's own P99 is reported informationally, not gated on — no calibrated threshold exists for it (its consensus has a different latency profile than the EVM chains the existing threshold was tuned against), so it isn't folded into a fabricated verdict. |
...
| Dependabot alerts (12: 1 high, 9 moderate, 2 low) | ✅ Partially fixed (2026-07-21). 10 of the 12 (1 high + 9 moderate) were `axios` 1.16.0, pulled in transitively via `@coinbase/x402` → `@coinbase/cdp-sdk` in `ts-agent/package.json` — not a direct dependency. Forced the patched `^1.18.0` via `overrides` (same mechanism already used there for `ws`), verified with a fresh install (axios resolves to 1.18.1) and the full test suite (21/21 still passing). The remaining 2 (low severity, `elliptic`, CVE-2025-14505, in both `ts-agent` and `sdk-typescript` via `casper-js-sdk`) have no non-breaking fix — `npm audit fix --force` wants to jump `casper-js-sdk` from `^3.0.0-rc05` to `5.0.12`, a major version bump on the exact SDK version this session spent significant effort getting right for Casper's entity-model addressing (see the sdk-typescript fix above). Not worth the risk for a low-severity advisory on a currently-working, verified system — left as a known, disclosed gap. |
...
| `casper-agent/rwa-settlement-gate/` — Odra contract | ✅ DEPLOYED (2026-07-29) to Casper Testnet: `contract-package-fab9c0a11314515796efddc5f5f98e0681cbdc717a2787a75a313cb5cb42511d`, tx `6dc5440f5516b9084700bfaa5fe7d63715a068c16dfcba3281994272a77b2a47`. Took 3 real attempts to get right, documented honestly: (1) built without `cargo odra build`'s module-name wiring correct — `Odra.toml` still pointed at the scaffold's original `flipper::Flipper` path after renaming the module, so the WASM's `call()` export was gated off (`ODRA_MODULE` cfg never matched) — cost 200 CSPR, no contract created; (2) old system `wasm-opt` (v105) couldn't lower `bulk-memory-operations` that modern LLVM's precompiled `core`/`alloc` emit by default on wasm32, and `-C target-cpu=mvp` alone doesn't fix it (only affects code compiled in *this* crate, not the precompiled std bundled by rustup) — fixed by fetching binaryen v128 directly from GitHub releases (not available via apt on this Ubuntu version); (3) pointed `ODRA_CASPER_LIVENET_NODE_ADDRESS` at `node.testnet.cspr.cloud` — `odra-casper-livenet-env` 2.9.0 has a real upstream regression where `CSPR_CLOUD_AUTH_TOKEN` is read into config but never attached to any HTTP request (confirmed against the pinned `release/2.9.0` source: zero `.header()` calls in the whole rpc-client crate), so every CSPR.cloud call fails silently under a genuinely unhelpful wrapper error (`"Livenet execution error"` — a `thiserror` format-string bug at `error.rs:10` drops the real message); switching to the official node (`https://node.testnet.casper.network`, same one `ts-agent`'s hand-rolled deploy script already uses) fixed it outright — first real attempt against it then failed on "Out of gas" at 200 CSPR, second succeeded at 500 CSPR. **Disk hygiene note:** Odra's `cargo odra test`/`cargo odra build` on this 25GB droplet twice drove `/` to <15MB free mid-build (`casper-storage`'s dependency tree is far heavier than the raw-WASM oracle contract's) — cleaned unrelated stale build caches (`/opt/casper-oracle/oracle-contract/target/`, 1.1GB, confirmed dead — no systemd unit references it) and an abandoned rustup toolchain to recover headroom; live services were never down, but came close to disk-full twice. ✅ NOW WIRED (2026-07-30): `ts-agent/publish_gate.js` publishes to the gate every 15 min (cron, independent of the oracle `update()` service). First publish verified on-chain: deploy `5e5a89bc894de0ffed286b4e9c6ba7a4aa90bdc1d39feb4e65a288471f59d111` executed Success (~1.58 CSPR net after refund), gate now holds `network_safe=true / identity_screening_active=true` — no longer default. |
