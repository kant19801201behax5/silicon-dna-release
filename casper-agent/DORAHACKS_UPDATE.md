...
- **Payments:** x402 protocol, currently via Base mainnet. Native Casper x402 payer built & verified (`casper-agent/casper-x402/`, signs against the live `x402-facilitator.cspr.cloud` testnet scheme) — going live is funding-gated (a CEP-18 stablecoin balance on the payer), not code
...
- A working CSPR.cloud API key is confirmed against the live facilitator, and the native Casper x402 payer is now **built and verified** (`casper-agent/casper-x402/`): from a clean `npm install` its self-test signs a valid EIP-712 payment authorization against the `casper:casper-test` scheme. Casper x402 settles in a CEP-18 stablecoin and signing needs no balance — so the payer is fully wired and will settle **automatically the moment the wallet holds the CEP-18 asset; no further code is required**. (We also patched an upstream `@make-software/casper-x402` v1.0.0 CJS load-crash — see that folder's README.) The existing Base-mainnet rail is deliberately left running untouched alongside it.
